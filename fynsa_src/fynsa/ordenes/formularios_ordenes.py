...
class rfi_ingreso_orden_formulario(forms.Form):
    ...
    duration_text = forms.MultipleChoiceField(choices=[('Toda la curva','Toda la curva'),('x<=3','x<=3'),('3<x<=5','3<x<=5'),('x>5','x>5')],initial='Toda la curva',required=False)
    ...
    country_of_risk = forms.MultipleChoiceField(choices = listado_cntry(),initial='Todos',required=False) 

    cliente.widget.attrs.update({'class':'form-control'})
    fecha.widget.attrs.update({'type':'date','class':'form-control'})
    ...
    country_of_risk.widget.attrs.update({'class':'form-control'})
